...
- Cleaning image: \
  `$ cosign clean slacktomcat/alpine_drogon@{manifest_digest}`
- Image signing: \
  `$ docker login && cosign sign --key ~/.cosign/cosign.key slacktomcat/alpine_drogon@{manifest_digest}`
- Verify signature: \
  `$ cosign verify --key ~/.cosign/cosign.pub slacktomcat/alpine_drogon@{mainfest_digest} | jq`
- Create BOM attestation for docker < 29.0:
  `$ syft slacktomcat/alpine_drogon@{manifest_digest} -o spdx-json > ./att.json`
  `$ docker login && cosign attest --type spdxjson --predicate ./att.json --key ~/.cosign/cosign.key` \\\
         `slacktomcat/alpine_drogon@manifest_digest`
- Create Provenance attestation for docker < 29.0: \
  `$ cosign attest --key ~/.cosign/cosign.key --type slsaprovenance --predicate provenance.json` \\\
         `slacktomcat/alpine_drogon@{manifest_digest}`
- Checking image tree: \
  `$ cosign tree slacktomcat/alpine_drogon@{manifest_digest}`
- Verify attestations signatures: \
  `$ cosign download attestation slacktomcat/alpine_drogon@{manifest_digest}  |  jq -r .payload | base64 -d  | jq .predicate` \
  `$ cosign verify-attestation --key ~/.cosign/cosign.pub --type spdxjson slacktomcat/alpine_drogon@{mainfest_digest}  |`  \\\
         `jq -r .payload |  base64 -d | jq .predicate` \
  `cosign verify-attestation --key ~/.cosign/cosign.pub --type slsaprovenance slacktomcat/alpine_drogon@{mainfest_digest}  | jq`
